...
class PotterybarnSpider(scrapy.Spider):
    ...
    productsAPI = f'https://core.dxpapi.com/api/v1/core/?q=XQUERYX&start=XSTARTX&rows=200&request_type=search&search_type=category&_br_uid_2=uid=59108974884:v=12.0:ts=1641921651974:hc=5&account_id=4068&url=https://www.potterybarn.com/&domain_key=potterybarn_en_pb_prd_d2&fq=inactive%3A%22false%22&fq=availability_logic%3A%22true%22&fl=pid%2Cprice%2Cprice_range%2Csale_price%2Csale_price_range%2Cthumb_image%2Ctitle%2CeligibleForQuickBuy%2CpipThumbnailMessages%2Cflags%2CisBopisOnly%2CimageOverride%2CaltImages%2Cstore_ids%2Ccategory_name_attr%2Ccategory_id_attr%2CpageLoadImages%2CimageRollOvers%2ChoverImages%2Cthumb_image_attr%2CleaderSkuImage%2CswatchesDisplay%2CmoreSwatchesCount%2CproductPriceType%2CmaxDiscountPercent&efq=store_ids:(%22ST%3A6223%22%20OR%20%22ST%3A0303%22%20OR%20%22ST%3A0296%22%20OR%20%22ST%3A0216%22%20OR%20%22ST%3A0429%22%20OR%20%22ST%3A0270%22%20OR%20%22ST%3A0448%22%20OR%20%22ST%3A6033%22%20OR%20%22ST%3A0962%22%20OR%20%22ST%3A0361%22%20OR%20%22ST%3A0378%22%20OR%20%22ST%3A0823%22%20OR%20%22ST%3A0058%22%20OR%20%22ST%3A0958%22%20OR%20%22ST%3A0276%22%20OR%20%22ST%3A0358%22%20OR%20%22ST%3A0538%22%20OR%20%22ST%3A0747%22%20OR%20%22ST%3A0371%22%20OR%20%22ST%3A0541%22%20OR%20%22ST%3A6018%22%20OR%20%22ST%3A0676%22%20OR%20%22ST%3A0960%22%20OR%20%22ST%3A6017%22%20OR%20%22ST%3A0540%22%20OR%20%22ST%3A0749%22%20OR%20%22ST%3A0526%22%20OR%20%22ST%3A6034%22)%20OR%20fulfillment_locations:(%22CRB%22%20OR%20%22CRB_CMO%22%20OR%20%22CRB_SS%22%20OR%20%22DEFAULT%22)'

    def parse(self, response):
        categories = ['https://www.potterybarn.com/shop/furniture/?cm_type=gnav']
    ...
    def parse_category(self, response):
        for sub_category in response.xpath('//a[@class="category-link"]/@href').getall():
            URL = 'https://www.potterybarn.com' + sub_category
            if '.html' in URL:
                continue
            if '/pages/' in URL:
                if '/m/' not in URL:
                    yield scrapy.Request(URL, callback=self.parse_sub_category, headers=self.HEADERS)
            elif '/shop/' in URL:
                if '/m/' not in URL:
    ...
    def parse_sub_category(self, response):
        URLs = response.xpath('//div/p/a[text()="Shop the Collection"]/@href').getall()
        if not URLs:
            URLs = response.xpath('//div[@class="module"]/a/@href').getall()
        for URL in URLs:
            if '/m/' not in URL:
                query = URL.split('/')[6] if len(URL.split('/')) > 6 else URL.split('/')[-1]
                query = query.split('--')[-1].strip('-').strip()
    ...
    def parse_products(self, response):
        ...
        for product in json_response['response']['docs']:
            URL = 'https://www.potterybarn.com/products/' + product['pid']
    ...
    def parse_product(self, response):
        ...
        for i in range(5):
            ...
            if image:
                image = image.replace('/rk/', '/ab/').replace('f.jpg', 'o.jpg')
                images.append(image)  
        if not images:
            images = [response.xpath('//img[@id="hero"]/@src').get()]   
        
        dimensions = ''
        all_dims = response.xpath('//div[@class="dream-pip-dimensions-list"]/div/ul')
        for dimension in all_dims.xpath('.//li/text()').getall():
        ...
        details = details.replace('PRODUCT DETAILS', '').strip()
        ...
        yield {
            ...
            'Sub Category 3': categories[3] if len(categories) > 2 else '',
